...
public class Tree extends Code {
  ...
  static final int vert_inter_elem_space = 4;
  static final int horiz_atom_tree_space = 4;
  static final int dotted_line_space = 5; // odd
  static final int horiz_tree_child_space = 4;
  static final int flat_inter_elem_space = 5;
  static final int flat_bracketsize = 4;
  static final int flat_nexttobracket = 5;
  static final int cons_bracketsize = 4;
  static final int cons_nexttobracket = 5;
  static final int cons_barsize = 2;
  ...
  Code child(int i) {
    return ((Branch) c.elementAt(i)).e;
  }

  Code add(Code e) {
    c.addElement(new Branch(e));
    return this;
  }
  ...
  void genpat(Sym s) throws jasError {
    s.genpat(this);
  }

  void genexp(Sym s) throws jasError {
    s.genexp(this);
  }

  void genpatpre(Sym s) throws jasError {
    s.genpatpre(this);
  }

  void genexppre(Sym s) throws jasError {
    s.genexppre(this);
  }

  boolean iscons() {
    return c.size() == 2 && h.s.compareTo("cons") == 0;
  }

  boolean isnil() {
    return c.size() == 0 && h.s.compareTo("nil") == 0;
  }
  ...
  void draw(Drawinfo d, int x, int y, boolean parcons) {
    ...
    if (drawmode == mtree) {
      ...
      for (int a = 0; a < c.size(); a++) {
        ...
        if (b.e.drawmode == mflatbr)
          d.bracket(
              b.x + x - flat_bracketsize - flat_nexttobracket, yp, flat_bracketsize - 1, xdot);
        b.e.draw(d, b.x + x, b.y + y, false);
        if (b.e.drawmode == mflatbr)
          d.bracket(
              b.x + x + b.e.xs + flat_bracketsize + flat_nexttobracket,
              yp,
              -flat_bracketsize + 1,
              xdot);
      }
