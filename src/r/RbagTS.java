...
public class RbagTS extends UnicastRemoteObject implements RbagIn {
  ...
  synchronized public Rval[] nfarray() throws RemoteException {
    ...
    for(int i = 0;i<normals.length;i++) {
      ...
      if(t!=null) {
        ...
        if(t.rejects!=null) for(int j = 0;j<t.rejects.size();j++) a[cur++] = (Rval)t.rejects.elementAt(j);
      };
    };
  ...
  synchronized public void addactive() throws RemoteException {
    stillactive++;
    if(dbg) System.out.println("addactive: "+stillactive);
  }

  synchronized public void addnormal(Rval x) throws RemoteException {
  ...
  synchronized public void waitnormalform() throws RemoteException {     // used by bag exp
    if(dbg) System.out.println("waitnormal");
    while(stillactive!=0) try { wait(); } catch(InterruptedException e) {};
  }

  synchronized public Rval[] grab(int id[]) throws RemoteException {
    if(dbg) System.out.println("grab start: "+id);
    start: for(;;) {
      if(nf) return null;
      if(busy) try { if(dbg) System.out.println("grab busy"); wait(); } catch(InterruptedException e) { continue start; };
  ...
  synchronized public void reject(Rval v[], int rejected) throws RemoteException {
  ...
  synchronized public void unreject(Rval v[]) throws RemoteException {
  ...
  public Rval statebuiltin(String op, Rval a, Rval b, Rval c, Rval d, Rval e) throws RemoteException {
