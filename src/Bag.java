...
public class Bag extends Code {
  ...
  static final Color bagcolor = new Color(200, 200, 255);

  Code add(Code x) {
    c.addElement(new BagElem(x));
    return this;
  }
  ...
  Code child(int i) {
    return ((BagElem) c.elementAt(i)).e;
  }
  ...
  Tree headtree() {
    return io == null ? null : io.headtree();
  }

  Bag headin() {
    return io == null ? null : this;
  }
  ...
  void draw(Drawinfo d, int x, int y, boolean parcons) {
    ...
    for (int a = 0; a < c.size(); a++) {
      BagElem be = (BagElem) c.elementAt(a);
      if (be.y + y + be.e.ys > 0 && be.y + y < d.gys)
        be.e.draw(d, be.x + x, be.y + y - vskip, false);
    }
    d.unshared(this);
  }
