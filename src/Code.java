...
public class Code implements Serializable {
  ...
  void dec() {
    refc--;
    if (refc == 0) for (int a = 0; a < nchildren(); a++) child(a).dec();
  }
  ...
  void genpat(Sym s) throws jasError {
    throw new CompError("no genpat");
  }

  void genexp(Sym s) throws jasError {
    throw new CompError("no genexp");
  }

  void genpatpre(Sym s) throws jasError {
    throw new CompError("no genpatpre");
  }

  void genexppre(Sym s) throws jasError {
    throw new CompError("no genexppre");
  }
  ...
  Code find(Findinfo f, int x, int y) {
    return hit(f.fx, f.fy, x, y) ? this : null;
  }
  ...
  void precalcsize() {
    bborder = (refc > 1 || isexample || warnval > 0) ? bborder_size : 0;
  }
  ...
  int speclevel() {
    return isexample ? 0 : 2;
  } // var = 0, tree = 1, rest = 2

  int speccomp(Code c) {
    return speclevel() - c.speclevel();
